...
public class ClientHandler extends Thread {
	...
	public void run() {
		...
		if (Server.clients.size() >= 2) {
			Server.broadcast(
					"Vous pouvez vous échanger vos clés publiques Tapez # + [ENTER] pour générer une clé publique à l'autre interlocuteur");
			// Server.broadcast("Ap");
			// Server.broadcast(Constantes.motifPK);
		}
		...
		while (true) {
			try {
				...
				if (msg != null) {
					System.out.println("Message recu du client : " + msg);
